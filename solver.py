    ...
    *,
    ...
    for employee_id in constrained_employees:
        for idx in range(len(dates) - 2):
            d1 = dates[idx]
            d3 = dates[idx + 2]
            # If the employee explicitly asked to work on both endpoint days,
            # do not penalize this specific work-rest-work gap.
            if (
                preferences.get((employee_id, d1)) == "prefer_work"
                and preferences.get((employee_id, d3)) == "prefer_work"
            ):
                continue
